...
按照题目给的五个层级逐级实现，**每一级都配一份 README 记录思路、超参数、逐轮训练日志和踩坑过程**。

- **LEVEL0~LEVEL3** 是打基础：环境自检 → MLP → CNN → AlexNet / ResNet，任务都是识别。
- **LEVEL4** 是真正的挑战：**每个像素都要预测**，用 U-Net 把手写答案、批注、演算从试卷照片里擦掉，同时保住印刷文字和表格线。

代码风格统一：**扁平结构、中文分段注释（`#准备数据集` / `#定义模型` / `#开始训练` / `#测试模型`）、无复杂抽象**，方便直接阅读。
...
## 主要踩坑（都写在各自的 README 里）

| 问题 | 原因 | 修复 |
| :--- | :--- | :--- |
| **AMP + SSIM 损失梯度全 NaN** | fp16 下算方差 `E[x²]-E[x]²` 下溢成 0/负数 | 损失在 autocast **外面**算（强制 fp32） |
| **多进程 DataLoader 起不来** | numpy memmap 默认按值 pickle，2.8 GB 撑爆管道 | 加 `__getstate__/__setstate__`，worker 内按路径重开 mmap |
| **逐张评估慢 670 倍** | `cudnn.benchmark=True` 对每个新输入尺寸重跑算法基准 | 全程关闭；测速前先热身 + `cuda.synchronize()` |
| **对比图里 GT 变纯白** | uint8 的 0~255 用了 `vmax=1` | `vmin/vmax` 跟着张量值域走 |
| **MNIST 反复重新下载** | 训练集/测试集用了不同的 `root` | 统一 root（`_check_exists()` 只认传入的那个） |
