...
def create_dataset():
    ...
    train_dataset = MNIST(root=DATA_DIR, train=True, transform=ToTensor(), download=True)
    test_dataset = MNIST(root=DATA_DIR, train=False, transform=ToTensor(), download=True)
...
class MLP(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # 卷积块 1: 1*28*28 --conv--> HIDDEN1*28*28 --pool--> HIDDEN1*14*14
        self.conv1 = nn.Conv2d(1, HIDDEN1, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        # 卷积块 2: HIDDEN1*14*14 --conv--> HIDDEN2*14*14 --pool--> HIDDEN2*7*7
...
def train(train_dataset):
    ...
    torch.save(model.state_dict(), './model/level2_CNN.pth')   #保存数据
...
#绘制损失曲线图像
    ...
    save_path = os.path.join(DRAW_DIR, 'CNN_DRAW.png')
...
def evaluate(test_dataset):
    ...
    model.load_state_dict(torch.load('./model/level2_CNN.pth',map_location=device))
